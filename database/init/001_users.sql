...
SELECT 'CREATE ROLE data_writer WITH LOGIN PASSWORD ' || quote_literal(:'data_writer_pass')
WHERE NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'data_writer')\gexec
...
SELECT 'CREATE ROLE data_reader WITH LOGIN PASSWORD ' || quote_literal(:'data_reader_pass')
WHERE NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'data_reader')\gexec
